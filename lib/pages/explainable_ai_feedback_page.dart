...
class ExplainableAiFeedbackPage extends StatelessWidget {
  ...
  const ExplainableAiFeedbackPage({
    ...
    this.service = const MockStudentLearningService(),
  });

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<AiFeedbackData>(
      future: service.getFeedbackForConcept(
        studentId: studentId,
      ...
      builder: (context, snapshot) {
        ...
        return StudentAppShell(
          ...
          onSectionSelected: (section) =>
              _openSection(context, section, service),
          child: data == null
              ? const Center(child: CircularProgressIndicator())
              : _FeedbackContent(data: data, service: service),
        );
      },
    );
  }
  ...
  static void _openSection(
    ...
    if (section == StudentNavSection.dashboard) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => StudentDashboardPage(service: service),
        ),
      );
      return;
    }
