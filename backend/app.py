...
def submit_answers(sid):
    ...
    for ua in user_ans:
        ...
        if fmt=="fill_blank":
            ...
            selected_index=ua.get("selected_index",-1)
            student_answer=options[selected_index] if 0<=selected_index<len(options) else ""
        elif fmt=="short_answer":
            options=[];correct_index=-1;selected_index=-1
            correct_answer=str(q.get("expected_answer",""))
            student_answer=str(ua.get("answer_text","") or "")
        else:
            options=[];correct_index=-1;selected_index=-1
            correct_answer={sp.get("id"):sp.get("expected_answer","") for sp in q.get("sub_parts",[])}
            student_answer=ua.get("sub_answers",{}) or {}

        if isinstance(xai,dict):
            xai.setdefault("correct_answer",correct_answer)
            xai.setdefault("error_type","None" if is_correct else "Concept or calculation error")
        ...
        results.append({"question_id":qid,"question_text":q.get("question_text",""),"format":fmt,
            ...
            "options":options,"correct_index":correct_index,"selected_index":selected_index,
            "answer":answer_summary,"student_answer":student_answer,"correct_answer":correct_answer,
